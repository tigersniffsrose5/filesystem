...
{
    if ( hasInit() )
        throw runtime_error("无法初始化，该文件夹下已存在文件系统");             //运行时错误

    initUser();

    fs::create_directory(blocksDirPath);                                         //创建目录
...
bool hasInit()
{
    return hasUserInit() && fs::exists(blocksDirPath) && fs::exists(rootDirBlockFilePath) && fs::exists(pwdFilePath);
}
